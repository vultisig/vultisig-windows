...
export const getChainSpecific = async (
  ...
  if (coin.chain === Chain.Monero) {
    console.log('[getChainSpecific] Monero path - fetching fee estimate...')
    try {
      const typicalTxWeight = 1650
      const feeEstimate = await getFeeEstimate()
      console.log(
        '[getChainSpecific] Monero fee estimate:',
        JSON.stringify(feeEstimate)
      )
      ...
      console.log(
        '[getChainSpecific] Monero estimated fee:',
      ...
      console.error('[getChainSpecific] Monero fee estimate error:', err)
      throw err
    }
  }

...
import { reshareWithPlugin } from '@core/mpc/fast/api/reshareWithPlugin'
import { reshareWithVerifier } from '@core/mpc/fast/api/reshareWithVerifier'

export const ResharePluginActionProvider = ({ children }: ChildrenProp) => {
  ...
  const action = useCallback(async () => {
    ...
    await reshareWithVerifier({
      ...
      old_parties: signers,
      hex_chain_code: hexChainCode,
      local_party_id: generateLocalPartyId('verifier'),
      old_reshare_prefix: resharePrefix ?? '',
      plugin_id: 'payroll-123',
    })

    await reshareWithPlugin({
      public_key: hasServer(signers) ? publicKeys.ecdsa : undefined,
      session_id: sessionId,
      hex_encryption_key: hexEncryptionKey,
      email,
      ...
      old_parties: signers,
      hex_chain_code: hexChainCode,
      local_party_id: 'plugin-payroll-123',
      old_reshare_prefix: resharePrefix ?? '',
      plugin_id: 'payroll-123',
    })

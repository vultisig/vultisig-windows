...
export const TxSuccess = ({
  ...
  const [{ scanResult, scanUnavailable }] = useCoreViewState<'keysign'>()
  ...
  return (
    <>
      ...
      <VStack gap={8}>
        {scanResult?.validation?.status === 'Success' &&
          scanResult?.validation?.result_type === BlockaidResultTypes.Benign &&
          !scanUnavailable && (
            <div
              style={{
                display: 'flex',

...
export const handleGetBalances: ToolHandler = async (input, context) => {
  ...
  const tasks = context.coins
    ...
    .map(async coin => {
      ...
      const balanceResult = await attempt(async () => {
        const id =
      ...
      if ('error' in balanceResult) {
        const message =
          balanceResult.error instanceof Error
        ...
        coinInfo.balance_raw = balanceResult.data.toString()
        coinInfo.balance = formatBalance(balanceResult.data, coin.decimals)
      }

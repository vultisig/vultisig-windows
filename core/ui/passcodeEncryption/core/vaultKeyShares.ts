...
type ChainKeySharesInput = {
  keyShares: Partial<Record<Chain, string>>
  key: string
}
...
export const encryptChainKeyShares = ({
  keyShares,
  key,
}: ChainKeySharesInput) =>
  recordMap(keyShares as Record<Chain, string>, value =>
    encryptWithAesGcm({
      key,
...
export const decryptChainKeyShares = ({
  keyShares,
  key,
}: ChainKeySharesInput) =>
  recordMap(keyShares as Record<Chain, string>, value =>

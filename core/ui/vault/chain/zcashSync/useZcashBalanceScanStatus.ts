...
export const useZcashBalanceScanStatus = ({
  ...
  const refresh = useCallback(async () => {
    if (!publicKeyEcdsa || !isNativeGrpcAvailable()) {
      setStatus({
        ...initialStatus,
        loading: false,
        isSyncing: false,
    ...
    try {
      ...
      const safeChainTip =
        chainTip != null ? getZcashSaplingSafeScanHeight(chainTip) : null
      const syncTarget = scanData?.scanTarget ?? safeChainTip
      const isSyncing =
        !scanData ||

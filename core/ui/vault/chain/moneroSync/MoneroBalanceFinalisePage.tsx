...
export const MoneroBalanceFinalisePage = () => {
  ...
  useEffect(() => {
    ...
    ensureMoneroScanDataSynced({
      ...
      console.error('Monero sync failed:', scanError)
    })
  ...
  const resetScan = async () => {
    ...
    await ensureMoneroScanDataSynced({
      ...
      console.error('Monero forced sync failed:', scanError)
    })
  ...
  const metrics = [
    ...
    },
    {
      ...
      extra: (
        <Text>
          {scanData != null
            ? formatMoneroAtomicAmount(scanData.balance)
            : '— XMR'}
        </Text>
      ),
    },

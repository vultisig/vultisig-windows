export const ru = {
  ...
  signed_awaiting_broadcast: 'Подписано, ожидается трансляция.',
  signed_awaiting_broadcast_description:
    'Данная транзакция была подписана и возвращена токену dApp , который её запросил. Отправка в сеть зависит от токена dApp ; статус обновляется, как только сеть его обнаруживает.',
}

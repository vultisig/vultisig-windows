...
describe('scanBlocks lifecycle', () => {
  ...
  it('persists the terminal scan state when already synced to the chain tip', async () => {
    ...
    expect(finalSave.scanHeight).toBe(120)
    expect(finalSave.scanTarget).toBe(120)
  ...
  it('does not restart a completed forward scan when the birthday is learned later', async () => {
    ...
    expect(current.scanHeight).toBe(120)
    expect(current.scanTarget).toBe(120)
  ...
  it('resumes an in-progress birthday scan instead of restarting from birth height', async () => {
    ...
    expect(getBlockRange).toHaveBeenCalledWith(111, 120)
  })
})

...
export const getOutputDistribution = async (
  ...
  for (const daemonUrl of getMoneroDaemonUrls()) {
    try {
      ...
      lastError = error
      console.warn('Monero get_output_distribution failed:', {
        daemonUrl,
        fromHeight,
        toHeight,
    ...
    for (const windowSize of fallbackWindows) {
      ...
      try {
        ...
        lastError = error
        console.warn('Monero get_output_distribution fallback failed:', {
          daemonUrl,
...
export const getOuts = async (indices: number[]): Promise<OutputEntry[]> => {
  ...
  if (!Array.isArray(result?.outs)) {
    console.error('Monero get_outs unexpected response:', result)
    throw new Error(
...
export const submitRawTx = async (txHex: string): Promise<void> => {
  ...
  if (result.status !== 'OK') {
    console.error('Monero broadcast rejected:', result)
    throw new Error(

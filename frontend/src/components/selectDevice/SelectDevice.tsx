...
const SelectDevice: React.FC<KeygenQRCodeProps> = ({ devices, selectedDevices, setSelectedDevices }) => {
    ...
    return (
        <>
            <h3 className="mt-5 font-semibold">{t('keygen_peer_discovery_select_the_pairing_devices')}</h3>
            <div className="flex flex-wrap justify-center w-full items-center gap-10 my-2">
                {devices.map((device, index) => (
                    <div className="relative bg-[#061B3A] rounded-lg text-center w-[100px] p-3 rounded-lg hover:border hover:border-white">
                        <div className="inline-flex items-center">
                            <label className="p-3 rounded-full cursor-pointer" htmlFor={"device-" + index}>
                                <input type="checkbox" onChange={() => checkHandler(index)}
                                    ...
                                    id={"device-" + index} />
                                <span
                                    className="absolute right-1 top-2 text-white transition-opacity opacity-0 pointer-events-none   -translate-x-2/4 peer-checked:opacity-100">
                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-2 w-2" viewBox="0 0 20 20" fill="#000000"
                                ...
                                <img src="/assets/images/phone.svg" alt="phone" className="mx-auto" />
                                <b className="my-2">{device}</b>
                            </label>
                        </div>
                    </div>

...
func (s *Store) SaveVault(vault *Vault) (string, error) {
	...
	query := `INSERT OR REPLACE INTO vaults (id, name, public_key_ecdsa, public_key_eddsa, created_at, hex_chain_code, local_party_id, signers,reshare_prefix, "order", is_backed_up)
							  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
...
func (s *Store) GetVault(id string) (*Vault, error) {
	query := `SELECT id, name, public_key_ecdsa, public_key_eddsa, created_at, hex_chain_code, local_party_id, signers,reshare_prefix, "order", is_backed_up FROM vaults WHERE id = ?`
...
func (s *Store) GetVaults() ([]*Vault, error) {
	query := `SELECT id, name, public_key_ecdsa, public_key_eddsa, created_at, hex_chain_code, local_party_id, reshare_prefix, "order", is_backed_up FROM vaults`

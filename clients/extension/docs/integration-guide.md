...
5. [Supported Methods](#supported-methods)
   ...
   - [Ripple / XRPL](#ripple--xrpl-windowvultisigripple-and-windowvultisigxrpl)
...
- **`window.vultisig.xrpl`** plus a guarded GemWallet-compatible message bridge
  for XRPL dApps.
...
### Ripple / XRPL (`window.vultisig.ripple` and `window.vultisig.xrpl`)

`window.vultisig.ripple` is the generic Vultisig chain provider:
...
`window.vultisig.xrpl` is the native XRPL object API. Its method shapes match
the corresponding `@gemwallet/api` results while avoiding the compatibility
bridge:
...
- **XRPL / GemWallet:** the direct object API is always available at
  `window.vultisig.xrpl`. When `window.gemWallet` is unclaimed, the extension
...
| `window.vultisig.xrpl` | Native XRPL | Direct account, message-signing, and transaction API |

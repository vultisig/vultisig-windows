...
# Fail closed if JSON parsing fails
if ! echo "$INPUT" | jq -e . >/dev/null 2>&1; then
  echo "Hook error: failed to parse TOOL_INPUT JSON. Blocking for safety." >&2
  exit 2
fi
...
if echo "$FILE_PATH" | grep -qiE '\.(env|pem|p12)$|credentials\.|secret\.|keystore'; then
